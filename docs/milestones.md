...
> the unit of work.** Each is independently testable and must be _finished_
...
- [ ] `sentIntent` result code correctly maps to SENT vs FAILED\_\* (taxonomy).
...
`SmsTransport` rolls all _sent_ reports up into one `SendHandle` (all sent → SENT,
...
- [ ] Backoff resets only after a _stable_ READY.
...
delivery**: a caller-supplied _stable_ idempotency id (`agent/EventKeys`), buffered
