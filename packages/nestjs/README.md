...
A [NestJS](https://nestjs.com) module for [`@luno/core`](../core). It wires the
...
import { LunoModule, attachLunoWebSocket } from '@luno/nestjs';
...
enrols, drives `WS /ws` with the `@luno/testing` fake node through pair → handshake
