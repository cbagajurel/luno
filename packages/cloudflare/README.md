...
[`@luno/core`](../core). It proves the §4 **socketless/edge path**: a Worker isolate
...
import { lunoWorker } from '@luno/cloudflare';
...
- `POST /enroll` and `/enroll/status` → `luno.http.handle(request)` (a Worker
  `Request` is fetch-native, so no shim — same as [`@luno/hono`](../hono)).
...
in-memory fake `WebSocketPair` and the `@luno/testing` fake node through the §6
