...
An [Express](https://expressjs.com) adapter for [`@luno-oss/core`](../core). It exposes
...
the seam a fetch-native adapter like [`@luno-oss/hono`](../hono) never exercises — and
...
import { registerLunoEnroll, attachLunoWebSocket } from '@luno-oss/express';
...
Swap `memoryStore()` for [`@luno-oss/store-postgres`](../store-postgres) to persist.
...
the `WS /ws` socket with the `@luno-oss/testing` fake node, and runs pair → handshake →
