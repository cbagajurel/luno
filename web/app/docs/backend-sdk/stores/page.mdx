...
| Store | Package | Use |
| --- | --- | --- |
| `memoryStore()` | `@luno/core` | Development and tests. Loses everything on restart |
| `PostgresStore` | `@luno/store-postgres` | Production. Driver-agnostic |
...
<Callout type="info">
  `memoryStore()` lives inside `@luno/core` rather than in its own package. A
...
In `@luno/store-postgres`, `claim()` is a single conditional
...
`@luno/testing/store`, which includes a concurrency test that hammers `claim()`
...
describe('PostgresStore', () => {
  storeConformance(() => new PostgresStore(testPool))
})
```

`@luno/store-postgres` passes it — including a 60-way concurrent-claim test —
...
3. Run `storeConformance` against it.
