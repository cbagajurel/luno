...
qrUri      // 'luno://pair?v=1&u=…&c=ABCD-1234&s=ses_9f3&l=Acme'
```

`backendUrl` is what makes the payload scannable — it tells the node where to
...
- `maxEnrollments: 1` — one device per session
- `expiresInMs: 600000` — a ten-minute expiry
- `requireApproval: false` — no approval gate unless you ask for one
- `allowReplacement: false` — a known device may not silently re-enrol
