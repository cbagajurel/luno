...
void main() {
  group('LunoButton', () {
    ...
    testWidgets('idle button fires onPressed and shows its icon', (
      tester,
    ) async {
      var pressed = 0;
      await tester.pumpWidget(
  ...
  group('AsyncView', () {
    testWidgets('renders loading, then data', (tester) async {
      await tester.pumpWidget(
        _host(
          const AsyncView<int>(value: AsyncValue.loading(), data: _intText),
        ),
      );
      expect(find.byType(CircularProgressIndicator), findsOneWidget);

      await tester.pumpWidget(
        _host(const AsyncView<int>(value: AsyncData(42), data: _intText)),
      );
  ...
  testWidgets('GlassContainer renders its child without blur errors', (
    ...
    await tester.pumpWidget(_host(const GlassContainer(child: Text('glass'))));
